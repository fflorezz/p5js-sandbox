...
function FixedDensityPDS(options, rng) {
  if (typeof options.distanceFunction === 'function') {
    throw new Error(
      'PoissonDiskSampling: Tried to instantiate the fixed density implementation with a distanceFunction'
    )
  }
  ...
  this.deltaDistance = Math.max(
    0,
    this.maxDistance - this.minDistancePlusEpsilon
  )
...
FixedDensityPDS.prototype.directAddPoint = function (point) {
  ...
  for (dimension = 0; dimension < this.dimension; dimension++) {
    internalArrayIndex +=
      ((point[dimension] / this.cellSize) | 0) * stride[dimension]
  }
...
FixedDensityPDS.prototype.inNeighbourhood = function (point) {
  ...
  for (
    neighbourIndex = 0;
    neighbourIndex < this.neighbourhood.length;
    neighbourIndex++
  ) {
    ...
    for (dimension = 0; dimension < dimensionNumber; dimension++) {
      currentDimensionValue =
        ((point[dimension] / this.cellSize) | 0) +
        this.neighbourhood[neighbourIndex][dimension]
    ...
    if (internalArrayIndex !== -1 && this.grid.data[internalArrayIndex] !== 0) {
      ...
      if (
        squaredEuclideanDistance(point, existingPoint) < this.squaredMinDistance
      ) {
        return true
      }
    }
...
FixedDensityPDS.prototype.getAllPointsWithDistance = function () {
  throw new Error(
    'PoissonDiskSampling: getAllPointsWithDistance() is not available in fixed-density implementation'
  )
}
...
function VariableDensityPDS(options, rng) {
  if (typeof options.distanceFunction !== 'function') {
    throw new Error(
      'PoissonDiskSampling: Tried to instantiate the variable density implementation without a distanceFunction'
    )
  }
...
VariableDensityPDS.prototype.inNeighbourhood = function (point) {
  ...
  for (
    ...
    if (
      internalArrayIndex !== -1 &&
      this.grid.data[internalArrayIndex].length > 0
    ) {
      for (var i = 0; i < this.grid.data[internalArrayIndex].length; i++) {
        ...
        existingPointDistance =
          this.sampleDistance[this.grid.data[internalArrayIndex][i]]
        ...
        if (
          euclideanDistance(point, existingPoint) <
          this.minDistance + this.deltaDistance * dist
        ) {
          return true
        }
      }
...
VariableDensityPDS.prototype.next = function () {
  ...
  while (this.processList.length > 0) {
    ...
    for (tries = 0; tries < this.maxTries; tries++) {
      inShape = true
      distance =
        this.minDistancePlusEpsilon +
        this.deltaDistance *
          (currentDistance + (1 - currentDistance) * this.bias)
